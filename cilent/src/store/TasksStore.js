...
const loadTasksFromLocalStorage = () => {
  try {
    ...
    return stored ? JSON.parse(stored).map(taskData => new mTask(
      taskData.name,
...
const useTasksStore = create((set, get) => ({
  ...
  loadTasks: async (userId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to load tasks from server, falling back to local storage');
        const localTasks = loadTasksFromLocalStorage();
        set({ tasks: localTasks, isOnline: false });
      }
    } catch (error) {
      console.error('Error loading tasks from server, falling back to local storage:', error);
      const localTasks = loadTasksFromLocalStorage();
      set({ tasks: localTasks, isOnline: false });
    } finally {
      set({ isLoading: false });
  ...
  mergeLocalTasks: async (userId) => {
    ...
    try {
      ...
      if (mergedCount > 0) {
        toast.success(`Awesome! ${mergedCount} local task${mergedCount > 1 ? 's' : ''} synced to your account.`);
      }
      
    } catch (error) {
      console.error('Error merging local tasks:', error);
              toast.error('Oops! Couldn\'t sync your local tasks. They\'re still safe locally.');
    }
  },
  ...
  createTask: async (task) => {
    ...
    if (isOnline && userId) {
      try {
        ...
        console.error('Error creating task on server, keeping local only:', error);
      }
    } else {
  ...
  editTask: async (updatedTask) => {
    ...
    set((state) => {
      const updatedTasks = state.tasks.map((task) => {
        if (task.id === updatedTask.id) {
          return { ...task, name: updatedTask.name, description: updatedTask.description };
        }
        return task;
      });
    ...
    if (isOnline && userId) {
      try {
        ...
        console.error('Error updating task on server, keeping local only:', error);
      }
    }
  },
  ...
  updateIsDone: async (task, value) => {
    ...
    if (isOnline && userId) {
      try {
        ...
        console.error('Error toggling task on server, keeping local only:', error);
      }
    }
  },
  ...
  removeTask: async (taskToRemove) => {
    ...
    if (isOnline && userId) {
      try {
        ...
        console.error('Error deleting task on server, keeping local only:', error);
      }
    }
  },
  ...
  moveTask: async (id, newDate) => {
    ...
    if (isOnline && userId) {
      try {
        ...
        console.error('Error moving task on server, keeping local only:', error);
      }
    }
  },
